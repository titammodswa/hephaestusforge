...
public class EntityMeltingCategory implements IRecipeCategory<ModRecipes.EntityMeltingRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, ModRecipes.EntityMeltingRecipe recipe, IFocusGroup focuses) {
        SpawnEggItem.byId(recipe.entityType()).ifPresent(eggHolder ->
                builder.addInvisibleIngredients(RecipeIngredientRole.INPUT)
                        .addItemStacks(List.of(new ItemStack(eggHolder)))
        );
    ...
    public void draw(ModRecipes.EntityMeltingRecipe recipe, IRecipeSlotsView slots,
        ...
        EntityRenderHelper.render(graphics, 11, 5, 48, recipe.entityType(), Minecraft.getInstance());

        Font font = Minecraft.getInstance().font;
        String dmg = Float.toString(recipe.damage() / 2f);
        int x = 68 - font.width(dmg);
        graphics.text(font, Component.literal(dmg).withStyle(ChatFormatting.RED), x, 8, Color.RED.getRGB());
        graphics.text(font, Component.literal("♥").withStyle(ChatFormatting.RED),
                x + font.width(dmg) + 1, 8, Color.RED.getRGB());
    }
