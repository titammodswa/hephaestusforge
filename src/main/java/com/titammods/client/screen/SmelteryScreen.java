...
public class SmelteryScreen extends AbstractContainerScreen<SmelteryMenu> {
    ...
    private static final int HEAT_BAR_W = 4;
    ...
    protected void init() {
        try {
            var fH = AbstractContainerScreen.class.getDeclaredField("imageHeight");
    ...
    private int totalSlots()  {
        return blockEntity != null ? blockEntity.itemHandler.getSlots() : 0;
    }
    private int getCols()     { return SmelteryMenu.calcColumns(totalSlots()); }
    private int getTotalRows(){ return (int) Math.ceil((double) totalSlots() / getCols()); }
    private int getVisRows()  { return Math.min(getTotalRows(), MAX_VIS_ROWS); }
    private int hasScroll()   { return getTotalRows() > MAX_VIS_ROWS ? 1 : 0; }

    private int panelW() { return getCols() * SLOT_W + 9; }

    private int panelH() {
        return 4 + 7 + getVisRows() * SLOT_H + 7;
    }

    private int panelX() {
        return this.leftPos - panelW() - 0;
    }
    ...
    private int slotsBaseX() {
        return this.leftPos - 22;
    }

    private int slotsBaseY() {
        return panelY() + 7 + 1;
    }
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mx, int my, float partial) {
        ...
        if (visRows > 0) {
            ...
            for (int row = 0; row < fullRows; row++) {
                ...
                for (int col = 0; col < slotsInRow; col++) {
                    ...
                    int slotX = bx - col * SLOT_W;
                    if (realIdx < blockEntity.meltingState.length) {
                        drawHeatBar(graphics,
                                slotX - HEAT_BAR_W + 1 - -4, rowY + 1,
                                realIdx);
                    }
                }
            }
            ...
            if (hasScroll() == 1) {
                ...
                int tw = 6;
                int tx = px - tw - -8;
                int ty = py + 7, th = visRows * SLOT_H;
                graphics.fill(tx, ty, tx + tw, ty + th, 0xFF373737);
                graphics.fill(tx + 1, ty + 1, tx + tw - 1, ty + th - 1, 0xFF181818);
                int thumbH  = Math.max(10, th * visRows / totalRows);
        ...
        if (blockEntity.fuelCapacity > 0 && !blockEntity.currentFuel.isEmpty()) {
            ...
            renderFluidLayer(graphics, blockEntity.currentFuel,
                    x + 152, y + 32 + fh - fh2, 16, fh2);
        }
    ...
    @SuppressWarnings("removal")
    private void drawHeatBar(GuiGraphicsExtractor g, int x, int y, int realIdx) {
        if (realIdx >= blockEntity.meltingState.length) return;
        int state = blockEntity.meltingState[realIdx];
        int prog  = blockEntity.meltingProgress[realIdx];
        int max   = blockEntity.meltingTime[realIdx];

        if (state == 1) {
            blit(g, x, y, 185, 150, 3, 16);
        } else if (state == 2) {
            blit(g, x, y, 179, 150, 3, 16);
        } else if (state == 3) {
            blit(g, x, y, 182, 150, 3, 16);
        } else if (state == 0 && max > 0 && prog > 0) {
            int bh = (int)(((float) prog / max) * 16);
            blit(g, x, y + (16 - bh), 176, 150 + (16 - bh), 3, bh);
        }
    }

    private void drawDynamicBorder(GuiGraphicsExtractor g, int x, int y, int w, int h) {
        g.fill(x + 7, y + 7, x + w, y + h - 7, 0xFFC6C6C6);
        blit(g, x, y,         0, 0,   7, 7);
            ...
            if (cap > 0 && !fluids.isEmpty()) {
                ...
                for (int i = 0; i < fluids.size(); i++) {
                    ...
                    if (my >= cy && my < cy + heights[i]) {
                        graphics.setComponentTooltipForNextFrame(font,
                                fluidTooltip(fluids.get(i), cap, shiftHeld), mx, my);
                        hit = true;
                        break;
                    }
                }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (hasScroll() == 1 && event.button() == 0) {
            int tw2 = 6; int tx = panelX() - tw2 - -8, ty = panelY() + 7, th = getVisRows() * SLOT_H;
            if (mx >= tx && mx < tx + 6 && my >= ty && my < ty + th) {
                isScrolling = true;
    ...
    private void updateScrollProgress(double my) {
        ...
        int ty     = panelY() + 7;
        int th     = visRows * SLOT_H;
        int thumbH = Math.max(10, th * visRows / totalRows);
        int travel = th - thumbH;

        scrollProgress = (float)(my - ty - thumbH / 2.0) / travel;
        scrollProgress = Math.max(0f, Math.min(scrollProgress, 1f));
        syncScroll(Math.round(scrollProgress * (totalRows - visRows)));
    }

    private void syncScroll(int row) {
